...
**Symptoms**: Browser shows SSL warnings or certificate errors

**Diagnosis**:
```bash
# Check certificate status
curl -vI https://[SERVICE].localhost

# Check Traefik certificate resolver
docker logs traefik | grep -i certificate

# Inspect Let's Encrypt storage
docker exec traefik ls -la /letsencrypt/
```

**Solutions**:

1. **Self-Signed Certificates for Development**
   ```bash
...
1. **Services on Wrong Network**
   ```yaml
   networks:
     - traefik    # For web access
...
# Test connection
docker exec -it postgresql psql -U platform -d platform

# Check database users
docker exec -it postgresql psql -U platform -c "\du"
```
...
# Test authentication
docker exec -it redis redis-cli -a redis_secure_password ping
```
...
2. **Redis Not on Correct Network**
   ```yaml
   networks:
     - database  # Ensure Redis is on database network
   ```
...
**Symptoms**: Services respond slowly to requests

**Diagnosis**:
```bash
# Check response times
curl -w "@curl-format.txt" -o /dev/null https://[SERVICE].localhost

# Monitor network latency
docker exec -it [SERVICE] ping -c 4 [TARGET]

# Check application logs for slow queries
docker logs [SERVICE] | grep -i "slow\|timeout"
```
...
**Symptoms**: Cannot find service logs
...
2. **Configure Log Volume**
   ```yaml
   volumes:
     - platform_logs:/var/log/[SERVICE]
   ```
...
**Symptoms**: Health checks or metrics not available
...
2. **Enable Metrics Endpoint**
   ```yaml
   labels:
     - "traefik.http.routers.[SERVICE]-metrics.rule=Host(`[SERVICE].localhost`) && Path(`/metrics`)"
   ```
...
3. **Check Prometheus Configuration**
   ...
   scrape_configs:
     - job_name: 'platform-services'
       static_configs:
         - targets: ['service1:port', 'service2:port']
   ```
...
**When to use**: Platform is completely broken and needs full reset
...
**When to use**: Single service is broken but platform is healthy

```bash
# 1. Stop the problematic service
docker compose -f domains/[DOMAIN]/[SERVICE]/compose.yml down

# 2. Remove any corrupted data
docker volume rm [SERVICE]_data

# 3. Pull fresh image
docker compose -f domains/[DOMAIN]/[SERVICE]/compose.yml pull
...
# Backup volumes
docker run --rm -v platform_logs:/source -v "$BACKUP_DIR":/backup alpine tar czf /backup/logs.tar.gz -C /source .
docker run --rm -v postgres_data:/source -v "$BACKUP_DIR":/backup alpine tar czf /backup/postgres.tar.gz -C /source .
...
# Restore volumes
docker run --rm -v platform_logs:/target -v "$BACKUP_DIR":/backup alpine tar xzf /backup/logs.tar.gz -C /target
docker run --rm -v postgres_data:/target -v "$BACKUP_DIR":/backup alpine tar xzf /backup/postgres.tar.gz -C /target
...
```bash
# Platform logs
./scripts/health-check.sh --json > platform-status.json

# Service logs
docker logs traefik > traefik.log 2>&1
docker logs nginx > nginx.log 2>&1
docker logs postgresql > postgresql.log 2>&1
...
- **Service-specific documentation**: Check individual container documentation

### Common Commands Reference

```bash
# Service management
./scripts/deploy.sh [SERVICE-PATH]           # Deploy service
./scripts/health-check.sh [SERVICE]          # Check health
./scripts/cleanup.sh [SERVICE]               # Remove service

# Docker commands
docker compose ps                            # List services
docker compose logs [SERVICE]               # View logs
docker compose restart [SERVICE]            # Restart service
docker compose pull [SERVICE]               # Update image

# Network troubleshooting
docker network ls                           # List networks
docker network inspect traefik              # Inspect network
docker exec -it [SERVICE] /bin/sh          # Access container
...
This troubleshooting guide should help you diagnose and resolve most common issues with the platform. Keep this guide updated as you encounter and resolve new issues.